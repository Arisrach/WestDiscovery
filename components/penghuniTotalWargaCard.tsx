...
export default function penghuniTotalWargaCard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const rawValue = rows[139]?.[2] ?? "Tidak ditemukan"; // D16 ada di index [15][3]
  ...
  return (
    <div className="flex items-center p-4 bg-yellow-400 border-2 border-black rounded-md shadow-[8px_8px_0px_#000]">
      <div className="p-3 bg-black rounded-full">
        <Users className="w-6 h-6 text-yellow-400" />
      </div>
      <div className="ml-4">
        <h2 className="text-md font-thin text-gray-500">Total Warga</h2>
